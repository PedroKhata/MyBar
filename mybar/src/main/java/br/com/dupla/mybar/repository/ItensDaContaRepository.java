...
public interface ItensDaContaRepository extends JpaRepository<ItensDaConta, Long> {
    ...
    // UC1 - Listar itens filtrando apenas pela conta correta
    ...
    @Query("SELECT i FROM ItensDaConta i WHERE i.ativo = true AND i.itemCardapio.tipo.cozinha = true AND i.dataRecebimentoCozinha IS NULL")
    List<ItensDaConta> findPedidosSolicitados();

    // UC7 - Cozinha: pedidos em preparo
    @Query("SELECT i FROM ItensDaConta i WHERE i.ativo = true AND i.itemCardapio.tipo.cozinha = true AND i.dataRecebimentoCozinha IS NOT NULL AND i.dataEntregaCozinha IS NULL")
    List<ItensDaConta> findPedidosEmPreparo();

    // UC8 - Balcão: pedidos prontos (ainda não recebidos pelo bar)
    @Query("SELECT i FROM ItensDaConta i WHERE i.ativo = true AND i.itemCardapio.tipo.cozinha = true AND i.dataEntregaCozinha IS NOT NULL AND i.dataRecebimentoBar IS NULL")
